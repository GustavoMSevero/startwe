app.controller("editarPerfilStartupCtrl", [
  "$scope",
  "$http",
  "$window",
  "$location",
  ...
  function ($scope, $http, $window, $location, $rootScope, $routeParams) {
    ...
    console.log(
      `id usuario ${$scope.idusuario}, usuario ${$scope.usuario}, id startup ${$scope.idstartup}`
    );
    ...
    if (location.hostname == "localhost") {
      ...
      var urlUploadPhotoStartup =
        "http://localhost:8888/web/startWe/api/uploadFotoStartup.php?idstartup=";
    } else {
      var urlPrefix = "";
    }
    ...
    var pegarStartup = function () {
      ...
      $http
        .get(urlOptionPrefix + option + "&idstartup=" + idstartup)
        .then(function (response) {
